...
class PostgresStarterBenchRepository(connection: Connection) extends AStarterBenchRepository {
  val logger: Logger = Logger(this.getClass)

  val qInsert: String = "INSERT INTO bench(id, is_ready) VALUES(?, ?) ON CONFLICT (id) DO UPDATE SET is_ready=false"
  val qGetIsReady: String = "SELECT is_ready FROM bench WHERE id = 'starter';"
  val qTruncate: String = "TRUNCATE bench CASCADE;"
  ...
  override def insertStarterBench(id: String, b: Boolean): Int = {
    ...
    preparedInsert.setString(1,  id.toString)
